...
@implementation RecommenTableViewCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        CGFloat weight = [UIScreen mainScreen].bounds.size.width;
        self.hero1Img = [[UIImageView alloc]initWithFrame:CGRectMake(10, 20, weight/6, weight/6)];
        _hero1Img.backgroundColor = [UIColor blueColor];
        [self addSubview:_hero1Img];
        
        self.hero2Img = [[UIImageView alloc]initWithFrame:CGRectMake(weight/6+20, 20, weight/6, weight/6)];
        _hero2Img.backgroundColor = [UIColor blueColor];
        [self addSubview:_hero2Img];
        
        self.hero3Img = [[UIImageView alloc]initWithFrame:CGRectMake((weight/6+10)*2+10, 20, weight/6, weight/6)];
        _hero3Img.backgroundColor = [UIColor blueColor];
        [self addSubview:_hero3Img];
        
        self.hero4Img = [[UIImageView alloc]initWithFrame:CGRectMake((weight/6+10)*3+10, 20, weight/6, weight/6)];
        _hero4Img.backgroundColor = [UIColor blueColor];
        [self addSubview:_hero4Img];
        
        self.hero5Img = [[UIImageView alloc]initWithFrame:CGRectMake((weight/6+10)*4+10, 20, weight/6, weight/6)];
        _hero5Img.backgroundColor = [UIColor blueColor];
        [self addSubview:_hero5Img];
        
        self.reason = [[UILabel alloc]initWithFrame:CGRectMake(10, weight/6 + 10, weight/4, weight/7)];
        _reason.text = @"推荐原因";
        _reason.textAlignment = NSTextAlignmentCenter;
        _reason.font = [UIFont systemFontOfSize:22];
        [self addSubview:_reason];
        
        self.desLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, weight/4+20, weight-20, weight/2)];        
